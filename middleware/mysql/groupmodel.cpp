...
vector<Group> GroupModel::queryGroups(string userid)
{
    ...
    */
    char sql[1024] = {0};
    sprintf(sql, "select a.groupid,a.groupname,a.groupdesc from group_ a inner join groupuser_ b on a.groupid = b.groupid where b.userid='%s'", userid.c_str());
    ...
    for (Group &group : groupVec)
    {
        ...
        if (res != nullptr)
        {
            ...
            while ((row = mysql_fetch_row(res)) != nullptr)
            {
                ...
                // 使用 fprintf() 写入字符串
                fprintf(fp, "groupid:%s,userid:%s,role:%s.\n", group.getGroupId().c_str(), a.c_str(), b.c_str());

                group.getGroupUsers().emplace(user, b);
            }
            mysql_free_result(res);
        }
    }
