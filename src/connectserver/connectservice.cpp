...
MsgHandler ConnectService::getHandler(int msgid)
{
    ...
    {
        // 返回一个默认的处理器，空操作
        return [=](const muduo::net::TcpConnectionPtr &conn, string &requeststr, muduo::Timestamp) {
        };
    }
    else
    {
...
// 处理客户端异常退出
void ConnectService::clientCloseException(const muduo::net::TcpConnectionPtr &conn)
{
    User user;
    {
...
// 处理注销业务
void ConnectService::Loginout(const muduo::net::TcpConnectionPtr &conn, string &requeststr, muduo::Timestamp time)
{
...
void ConnectService::Login(const muduo::net::TcpConnectionPtr &conn, string &requeststr, muduo::Timestamp time)
{
    ...
    if (loginresponse.result().success())
    {
        ...
        _redis.subscribe(stoi(loginrequest.userid()));
    }
...
void ConnectService::Register(const muduo::net::TcpConnectionPtr &conn, string &requeststr, muduo::Timestamp time)
{
    fixbug::UserServiceRpc_Stub stub(new MprpcChannel());
...
// 一对一聊天业务
void ConnectService::OneChat(const muduo::net::TcpConnectionPtr &conn, string &requeststr, muduo::Timestamp time)
{
    fixbug::OneChatRequest onechatrequest;
    onechatrequest.ParseFromString(requeststr);
    fixbug::OneChatResponse onechatresponse;

    fixbug::protobuffer buffer1;
    buffer1.set_protobuftype(ONE_CHAT_MSG);
    fixbug::ChatMessage cmsg = onechatrequest.chatmsg();
    buffer1.set_protobufstr(cmsg.SerializeAsString());
    string onechatmsg = buffer1.SerializeAsString();
    ...
    if (flag)
    {
        User user = UserModel::GetInstance().query(onechatrequest.friendid());
        if (user.getUserState() == "online")
        {
            _redis.publish(stoi(onechatrequest.friendid()), onechatmsg);
        }
        else
        {
            // 不在线，存储离线消息
            OfflineMsgModel::GetInstance().insert(onechatrequest.friendid(), cmsg.SerializeAsString());
        }
    }

    onechatresponse.mutable_result()->set_success(true);
    fixbug::protobuffer buffer2;
    buffer2.set_protobuftype(ONE_CHAT_ACK);
    buffer2.set_protobufstr(onechatresponse.SerializeAsString());
    string responsestr = buffer2.SerializeAsString();
    conn->send(responsestr);
}

// 群组聊天业务
void ConnectService::GroupChat(const muduo::net::TcpConnectionPtr &conn, string &requeststr, muduo::Timestamp time)
{
    ...
    // 群消息
    fixbug::protobuffer buffer1;
    buffer1.set_protobuftype(GROUP_CHAT_MSG);
    fixbug::ChatMessage cmsg = groupchatrequest.chatmsg();
    buffer1.set_protobufstr(cmsg.SerializeAsString());
    string groupchatmsg = buffer1.SerializeAsString();
    ...
    for (string &id : useridVec)
    {
        ...
        if (it != _userConnMap.end())
        {
            it->second->send(groupchatmsg);
        }
        else
        {
            User user = UserModel::GetInstance().query(id);
            if (user.getUserState() == "online")
            {
                _redis.publish(stoi(id), groupchatmsg);
            }
            else
            {
                // 不在线，存储离线消息
                OfflineMsgModel::GetInstance().insert(id, cmsg.SerializeAsString());
            }
        }
    }
    ...
    fixbug::protobuffer buffer2;
    buffer2.set_protobuftype(GROUP_CHAT_ACK);
    buffer2.set_protobufstr(groupchatresponse.SerializeAsString());
    string responsestr = buffer2.SerializeAsString();
    conn->send(responsestr);
}

// 获取当前用户的离线消息  个人聊天信息或者群组消息
void ConnectService::GetOfflineMsg(const muduo::net::TcpConnectionPtr &conn, string &requeststr, muduo::Timestamp time)
{
...
// 获取用户的所有好友信息
void ConnectService::GetFriendList(const muduo::net::TcpConnectionPtr &conn, string &requeststr, muduo::Timestamp time)
{
...
void ConnectService::AddFriend(const muduo::net::TcpConnectionPtr &conn, string &requeststr, muduo::Timestamp time)
{
    fixbug::FriendServiceRpc_Stub stub(new MprpcChannel());
...
// 创建群组业务
void ConnectService::CreateGroup(const muduo::net::TcpConnectionPtr &conn, string &requeststr, muduo::Timestamp time)
{
...
// 加入群组业务
void ConnectService::AddGroup(const muduo::net::TcpConnectionPtr &conn, string &requeststr, muduo::Timestamp time)
{
    fixbug::GroupServiceRpc_Stub stub(new MprpcChannel());
...
// 获取用户加入的所有群组信息
void ConnectService::GetGroupList(const muduo::net::TcpConnectionPtr &conn, string &requeststr, muduo::Timestamp time)
{
    fixbug::GroupServiceRpc_Stub stub(new MprpcChannel());
